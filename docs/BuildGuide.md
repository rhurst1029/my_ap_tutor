...
Since May 2026 the app supports a tutor-facing **guided practice** flow — a
timed, stage-by-stage walkthrough of a structured tutoring session.

### Frontend route

`http://localhost:5173/?guided=1` mounts `GuidedPractice`
(`frontend/src/components/GuidedPractice/GuidedPractice.tsx`). The plan is
Bella-specific for v1 (derived from her day-before-exam session plan), so the
route hardcodes `studentName = 'Bella'`.

### Plan data

`frontend/src/components/GuidedPractice/guidedPlan.ts` exports `GUIDED_PLAN`,
an array of `GuidedStage` objects. Each stage has `id`, `title`, `timeLabel`,
`durationMin`, `kind`, `purpose`, `body[]`, and optional `checklist[]`,
`testId`, `writingPrompt`. Stage `kind` is one of:
- `instruction` — purpose + instruction lines + checklist
- `warmup` — same layout, used for the retrieval warm-up
- `frq` — embeds `TestRunner` on `testId` (the FRQ drill) inline
- `break` — rest screen with a calm note
- `writing` — adds a `<textarea>` for the expressive-writing exercise

### Component behavior

- Per-stage countdown timer (`durationMin`), auto-starts on stage entry,
  Pause/Resume + Reset controls, clamps at 0 ("Time's up") without
  auto-advancing — the tutor controls pacing.
- Progress bar + stage dots; Previous/Next nav; "Finish ✓" on the last stage
  calls `onExit`.
- The embedded FRQ `TestRunner` is mounted once the FRQ stage is first reached
  and then kept alive via a `display` toggle — navigating away and back does
  NOT restart its backend session.
- Checklist tick-state and the writing-stage textarea both persist across
  stage navigation (keyed component state).
